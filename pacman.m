...
set(handles.PacMan,'units','points','Position',[2149.6 518.4 460.8 344.8]); 
...
global datapath SubjectName inum
inum = 1;
...
if ~iscell(SubjectName)
    ...
    if strcmp(SubjectName, 'Omega')
        folder = struct2table(dir('Omega'));
        while any(strcmp(strcat(SubjectName, '-', date, '-', num2str(inum)), cell(folder.name)))
            inum = inum+1;
        end
        current_path = [cd,'/Omega/'];
    elseif strcmp(SubjectName, 'Patamon')
        folder = struct2table(dir('Patamon'));
        while any(strcmp(strcat(SubjectName, '-', date, '-', num2str(inum)), cell(folder.name)))
            inum = inum+1;
        end
